...
/**
 * 异常日志捕获
 * Created by Maclaine on 2017/8/11.
 */
...
//        nameString = BmobUserManager.getInstance(mContext).getCurrentUserName();
    ...
    private boolean handleException(Throwable ex) {
        ...
        Log.e(TAG, Arrays.asList(ex.getStackTrace()).toString());
//        WonderMapApplication.getInstance().getSpUtil().setCrashLog(true);// 每次进入应用检查，是否有log，有则上传
        ...
        new Thread() {
            ...
            public void run() {
                Looper.prepare();
                Toast.makeText(mContext, "程序出现无法预料的异常，正在收集日志，即将退出或重启", Toast.LENGTH_LONG).show();
                Looper.loop();
            }
        }.start();
        // 收集设备参数信息
...
//        Log.d(WModel.CrashUpload, result);
        ...
        try {
            ...
            String fileName = "pointhouse" + "-" + time + "-" + timestamp
                    + ".log";
            if (Environment.getExternalStorageState().equals(
                ...
                if (SDCardHelper.isSDCardMounted()) {
                    if (GlobalApplication.sContext.getExternalFilesDir(null) != null) {
                        path = GlobalApplication.sContext.getExternalFilesDir(null) + "/";
                    } else {
                        path = GlobalApplication.sContext.getFilesDir() + "/";
                    }
                } else {
                    path = GlobalApplication.sContext.getFilesDir() + "/";
                }
