package com.pointhouse.chiguan.common.swipelistview;
    ...
    ViewDragHelper.Callback callback = new ViewDragHelper.Callback() {
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            if (releasedChild == itemView) {
                if (xvel == 0&& Math.abs(itemView.getLeft()) > hiddenViewWidth / 10) {//&& Math.abs(itemView.getLeft()) > hiddenViewWidth / 2.0f
                    Log.d(TAG,"xvel== 0");
                    open(smooth);
                } else if (xvel < 0) {
                    Log.d(TAG,"xvel < 0");
                    open(smooth);
                } else if(xvel>hiddenViewWidth/10){//防止误操作
                    Log.d(TAG,"xvel>0");
                    close(smooth);
                }else{
                    Log.d(TAG,xvel+"");
